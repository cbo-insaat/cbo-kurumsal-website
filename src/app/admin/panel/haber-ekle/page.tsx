...
// ✅ Quill HTML -> plain text length validation
...
export default function HaberEklePage() {
  ...
  const validate = async () => {
    ...
    if (!contentText || contentText.length < 20) return "İçerik en az 20 karakter olmalı.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "posts"), {
        ...
        content: form.content, // ✅ HTML kaydediyoruz
        status: form.status,
  ...
  const saveEdit = async (id: string) => {
    try {
      const title = (editDraft.title as string) || "";
      const excerpt = (editDraft.excerpt as string) || "";
      const content = (editDraft.content as string) || "";
      const status = editDraft.status as "draft" | "published" | undefined;

      if (!title.trim() || !excerpt.trim() || !stripHtml(content).trim() || !status) {
          ...
          <div>
            ...
            <div className="mt-2 rounded-lg border border-gray-300 overflow-hidden text-black">
              <ReactQuill
                theme="snow"
                value={form.content}
                onChange={(val: string) => setForm((p) => ({ ...p, content: val }))}
            ...
            <p className="mt-2 text-xs text-gray-500">
              Not: Bu alan HTML olarak kaydedilir. (Uzunluk kontrolü düz metin üzerinden yapılır.)
            </p>
          </div>
                  ...
                  <div key={p.id} className="rounded-2xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                    ...
                    {!isEditing ? (
                      <div className="p-4">
                        ...
                        {p.category && <p className="mt-0.5 text-xs text-gray-500">Kategori: {p.category}</p>}
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        {/* ✅ Edit içerik (ReactQuill) */}
                        <div className="rounded-lg border border-gray-300 overflow-hidden">
                          <ReactQuill
                            theme="snow"
                            value={(editDraft.content as string) || ""}
                            onChange={(val: string) => setEditDraft((d) => ({ ...d, content: val }))}
                            modules={quillModules}
                            formats={quillFormats}
                            placeholder="İçerik"
                          />
